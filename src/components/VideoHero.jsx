...
export default function VideoHero() {
  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (alive) { setImages(list); setBad(new Set()); setIdx(0) }
      } catch (err) {
  ...
  return React.createElement('section', { className: 'relative h-[80vh] md:h-[90vh] lg:h-screen overflow-hidden' },
    ...
    React.createElement('div', { className: 'absolute inset-0 bg-black' },
      live.length
        ? live.map((src, i) => {
            ...
            return React.createElement('picture', { key: src },
              React.createElement('source', { type: 'image/webp', srcSet: webpSrc }),
        ...
        : React.createElement('div', { className: 'absolute inset-0 bg-gray-300' })
    ),
    React.createElement('div', { className: 'absolute inset-0 bg-black/35' }),
    React.createElement('div', { className: 'relative z-10 flex flex-col items-center justify-center h-full text-center px-6' },
    ...
    live.length > 1 && React.createElement('div', { className: 'absolute bottom-4 left-0 right-0 z-10 flex justify-center gap-2' },
      live.map((_, i) => React.createElement('button', {
        key: i,
        onClick: () => setIdx(i),
