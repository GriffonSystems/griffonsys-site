...
const ALT_BY_FILE = {
  "hero-07a.jpg":
    "Indoor security camera installed in Illinois facility by Griffon Systems",
  "hero-02.jpg":
    "Wide-angle surveillance camera view of residential alley installed by Griffon Systems",
  "hero-03a.jpg":
    "Avigilon dome and bullet security cameras installed on commercial building in Chicago",
  "hero-04a.jpg":
    "Outdoor bullet security camera installed under building eave by Griffon Systems",
  "hero-05a.jpg":
    "Security camera installation at Illinois utility substation by Griffon Systems",
  "hero-01.jpg":
    "Overhead security camera footage of commercial parking area in Illinois",
  "hero-06a.jpg":
    "Verkada access control intercom and keypad installed at gated facility entrance",
}

export default function VideoHero() {
  const BASE = "/hero"
  const VER = import.meta.env?.VITE_ASSET_VERSION
    ? `?v=${encodeURIComponent(import.meta.env.VITE_ASSET_VERSION)}`
    : ""
  ...
  return React.createElement(
    "section",
    { className: "relative h-[80vh] md:h-[90vh] lg:h-screen overflow-hidden" },
    ...
    React.createElement(
      ...
      live.length
        ? live.map((src, i) => {
            ...
            const alt =
              ALT_BY_FILE[fname] ||
              "Security camera installation by Griffon Systems"
            const kbName = i % 2 === 0 ? "kbZoomIn" : "kbZoomOut"

            // ✅ IMPORTANT: no .webp <picture> here (you don't have webp files)
            return React.createElement("img", {
              ...
              className:
                "absolute inset-0 w-full h-full object-cover transition-opacity kb-anim will-change-transform",
              style: {
                objectPosition: pos,
    ...
    React.createElement("div", { className: "absolute inset-0 bg-black/35" }),
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        {
          className:
            "text-white/95 text-lg md:text-xl max-w-3xl font-medium drop-shadow-lg",
          style: {
            fontFamily:
              'Optima, Candara, "Noto Sans", system-ui, -apple-system, "Segoe UI", Roboto, Arial, sans-serif',
          },
    ...
    live.length > 1 &&
      React.createElement(
        ...
        {
          className:
            "absolute bottom-4 left-0 right-0 z-10 flex justify-center gap-2",
        },
        live.map((_, i) =>
